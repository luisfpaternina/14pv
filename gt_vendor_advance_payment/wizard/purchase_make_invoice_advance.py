		...
		for rec in self:
			...
			if rec.advance_payment_method == 'delivered':
				...
				if not rec.product_id:
					...
					rec.env['ir.config_parameter'].sudo().set_param('sale.default_deposit_product_id', rec.product_id.id)
				...
				for order in purchase_orders:
					...
					taxes = rec.product_id.taxes_id.filtered(lambda r: not order.company_id or r.company_id == order.company_id)
					if order.fiscal_position_id and taxes:
						tax_ids = order.fiscal_position_id.map_tax(taxes, rec.product_id, order.partner_shipping_id).ids
					else:
						tax_ids = taxes.ids
					...
					del context
					po_invoice_id = rec._create_invoice(order, po_line, amount)
